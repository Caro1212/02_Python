...
def plot_prediction(model, error, ts_fresh = False):
    if ts_fresh:
        ...
        axs[0].plot(h_compare.y_9, label="y")
        axs[0].plot(h_compare.y_dach, label="y_dach")
        e_h = rmse(h_compare.y_dach.values, h_compare.y_9.values)
        axs[0].set_title("Hammerstein - RMSE = " + str(e_h))
        axs[0].legend()

        axs[1].plot(w_compare.y_9, label="y")
        axs[1].plot(w_compare.y_dach, label="y_dach")
        e_w = rmse(w_compare.y_dach.values, w_compare.y_9.values)
        axs[1].set_title("Winer - RMSE = " + str(e_w))
        axs[1].legend()

        axs[2].plot(n_compare.y_9, label="y")
        axs[2].plot(n_compare.y_dach, label="y_dach")
        e_n = rmse(n_compare.y_dach.values, n_compare.y_9.values)
        axs[2].set_title("NDE - RMSE = " + str(e_n))
        axs[2].legend()
        fig.tight_layout()
